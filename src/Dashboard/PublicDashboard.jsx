...
const PublicDashboard = () => {
  ...
  const fetchUserOrders = async () => {
    if (user) {
      try {
        const response = await fetch('https://inventory-server-1-atx9.onrender.com/inventix/orders')
  ...
  const deleteOrder = async (orderId) => {
    if (confirm('Are you sure you want to delete this order?')) {
      try {
        const response = await fetch(`https://inventory-server-1-atx9.onrender.com/inventix/orders/${orderId}`, {
          method: 'DELETE'
        })
